...
export function useWebSocket({ onLog }: UseWebSocketParams) {
  ...
  const [connectionType, setConnectionType] =
    useState<ConnectionType>("websocket");
  ...
  const cancelConnection = useCallback(() => {
    ...
    onLog({
      type: "INFO",
      message: "Tentativa de conexão cancelada pelo usuário",
    });
  ...
  const connect = useCallback(
    (
      ...
      connectionTimeoutRef.current = setTimeout(() => {
        ...
        onLog({
          type: "ERROR",
          message: "Tempo limite de conexão atingido (30 segundos)",
        });
      ...
      if (type === "websocket") {
        ...
        try {
          ...
          ws.onclose = (event) => {
            ...
            if (timeoutOccurredRef.current) {
              console.log(
                "[WebSocket] Fechamento devido a timeout, ignorando log",
              );
        ...
        if (token) {
          ...
          connectHeaders["Authorization"] = token.startsWith("Bearer ")
            ? token
            : `Bearer ${token}`;
        ...
        if (Object.keys(connectHeaders).length > 0) {
          console.log(
            "[STOMP] Headers de conexão:",
            Object.keys(connectHeaders),
          );
        }
        ...
        const client = new Client({
          ...
          onConnect: (frame) => {
            ...
            onLog({
              type: "INFO",
              message: `STOMP conectado ao servidor: ${url}`,
            });
          },
          onStompError: (frame) => {
            clearConnectionTimeout();
          ...
          onWebSocketClose: (event) => {
            ...
            if (timeoutOccurredRef.current) {
              console.log(
                "[STOMP] Fechamento devido a timeout, ignorando log de tentativas",
              );
              timeoutOccurredRef.current = false; // Reseta a flag
              return;
            }
            ...
            console.log(
              `[STOMP] Tentativa de reconexão ${reconnectAttemptsRef.current}/3`,
            );
            setStatus("disconnected");
            onLog({
              type: "INFO",
          ...
          onWebSocketError: (event) => {
            ...
            onLog({
              type: "ERROR",
              message: `Erro na conexão WebSocket do STOMP`,
              // message: `Erro na conexão WebSocket do STOMP - Tentativa ${reconnectAttemptsRef.current + 1}/3`,
            });
          },
          onDisconnect: () => {
        ...
        try {
          ...
          onLog({
            type: "ERROR",
            message: `Erro ao ativar cliente STOMP: ${error}`,
          });
        }
      }
    },
    ...
    (destination: string) => {
      ...
      if (connectionType === "stomp") {
        ...
        if (!stompClientRef.current.connected) {
          onLog({
            type: "ERROR",
            message: "STOMP não conectado. Aguarde a conexão ser estabelecida.",
          });
          return;
        }
        ...
        try {
          ...
          onLog({
            type: "ERROR",
            message: `Erro ao se inscrever em ${destination}: ${error}`,
          });
        }
        ...
        onLog({
          type: "SUBSCRIBE",
          message: `Inscrição adicionada: ${destination} (WebSocket recebe todas as mensagens)`,
        });
      } else {
        onLog({
          type: "ERROR",
          message: "Não conectado. Conecte-se primeiro.",
        });
      }
    },
  ...
  const unsubscribe = useCallback(
    (id: string) => {
      ...
      if (topic) {
        onLog({
          type: "UNSUBSCRIBE",
          message: `Inscrição cancelada: ${topic.destination}`,
        });
      }
    },
      ...
      if (connectionType === "websocket") {
        ...
        if (wsRef.current.readyState !== WebSocket.OPEN) {
          onLog({
            type: "ERROR",
            message:
              "WebSocket não está aberto. Estado atual: " +
